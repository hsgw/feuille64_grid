...
inline uint8_t cdc_recieve_byte(void) {
    int ret = -1;
    while (ret < 0) {
        ret = CDC_Device_ReceiveByte(&VirtualSerial_CDC_Interface);
    }
    return (uint8_t)ret;
}
...
void serial_process(void) {
    while (true) {
        int16_t cmd_byte = CDC_Device_ReceiveByte(&VirtualSerial_CDC_Interface);
